...
/**
 * \file    custom/aeromigration/scripts/renumber_supplier_orders.php
 * \ingroup aeromigration
 * \brief   Renumérote les commandes fournisseur selon la règle arrêtée avec le client.
 *
 * ------------------------------------------------------------------------------
 * CE SCRIPT RÉÉCRIT LA RÉFÉRENCE DE ~2 570 COMMANDES FOURNISSEUR — LISEZ AVANT
 * ------------------------------------------------------------------------------
 *
 * ## La règle — celle des factures, validée par le client le 20/08/2026
 *
 * **Coupure au 1er octobre 2023** (début de l'exercice fiscal 2023-2024, les exercices vont
 * d'octobre à septembre). Avant, la pièce reprend son numéro ADD : `COF<millésime>-<chiffres
 * du numéro source>` — chiffres tels quels, lettres retirées, zéros conservés (CF7344 →
 * COF1920-7344 si la pièce date de l'exercice 2019-2020 ; BCF990000123 → COF…-990000123).
 * Après, séquence chronologique à `000001` par exercice, compteur six chiffres — celui que le
 * modèle `COF` d'aerotoolbox 1.16.0 continuera d'incrémenter.
 *
 * Le numéro source vient du `ref_ext` (`SAGE:<pièce>`), jamais de la référence actuelle :
 * celle-ci est hétéroclite — des `CF7344`, des `BCF…`, et même un `undefinedBCF9900` posé par
 * un incident d'écran — et la renumérotation l'assainit d'un coup. Les brouillons `(PROV…)`
 * sont ignorés ; ils sont d'ailleurs les seules pièces sans `ref_ext` de reprise.
 *
 * ## Documents : le garde-fou des factures
 *
 * Renuméroter sous des PDF nommés à l'ancienne référence les rendrait orphelins. Le script
 * REFUSE de tourner si des documents de commandes fournisseur subsistent (au 20/08/2026 :
 * trois fichiers d'essai sur l'instance de test, à supprimer d'abord — répertoires,
 * `llx_ecm_files` sous fournisseur/commande, `last_main_doc`).
 *
 * ## Écriture directe assumée, idempotence, contrôles
 *
 * Mêmes principes que `renumber_invoices.php` : UPDATE direct (le coeur n'offre aucune API de
 * renommage), cibles calculées de données stables donc rejouables, unicité totale vérifiée
 * avant toute écriture, pré-phase de déplacement des références qui occuperaient déjà une
 * cible, `--limit` pour un lot d'essai.
 *
 * Usage :
 *   php renumber_supplier_orders.php                 simulation
 *   php renumber_supplier_orders.php --confirm       applique
 *   php renumber_supplier_orders.php --limit=50 --confirm    lot d'essai
 */
...
/*
 * Garde-fou : aucun document de commande fournisseur ne doit subsister.
 */

$problems = array();

$resql = $db->query("SELECT COUNT(*) AS n FROM ".MAIN_DB_PREFIX."ecm_files"
    ." WHERE filepath = 'fournisseur/commande' OR filepath LIKE 'fournisseur/commande/%'");
if ($resql && ($obj = $db->fetch_object($resql)) && (int) $obj->n > 0) {
    $problems[] = $obj->n." entrée(s) llx_ecm_files sous fournisseur/commande/";
}
$resql = $db->query("SELECT COUNT(*) AS n FROM ".MAIN_DB_PREFIX."commande_fournisseur"
    ." WHERE last_main_doc IS NOT NULL AND last_main_doc <> ''");
if ($resql && ($obj = $db->fetch_object($resql)) && (int) $obj->n > 0) {
    $problems[] = $obj->n." commande(s) avec last_main_doc";
}
$dirOutput = !empty($conf->fournisseur->commande->dir_output) ? $conf->fournisseur->commande->dir_output : '';
...
if ($problems) {
    echo "REFUS : des documents de commandes fournisseur subsistent — renuméroter les rendrait orphelins.\n";
    foreach ($problems as $p) {
        echo "  - ".$p."\n";
    }
    echo "Supprimez d'abord les répertoires documents/fournisseur/commande/*, les lignes\n";
    echo "llx_ecm_files (filepath 'fournisseur/commande/%') et remettez last_main_doc à NULL.\n";
    exit(1);
}
 ...
 *
 * La date de référence est celle de la commande ; les rares pièces sans date de commande
 * retombent sur leur date de création — même chronologie que la reprise.
 */

$resql = $db->query('SELECT rowid, ref, ref_ext,'
    .' COALESCE(date_commande, date_creation) as d'
    .' FROM '.MAIN_DB_PREFIX.'commande_fournisseur'
    .' WHERE entity IN ('.getEntity('supplier_order').')'
...
foreach ($orders as $ord) {
    ...
    if (substr($ord->d, 0, 10) < CUTOFF) {
        ...
        $digits = preg_replace('/\D+/', '', substr($ord->ref_ext, strlen(REF_EXT_PREFIX)));
        if ($digits === '') {
...
$resql = $db->query('SELECT ref FROM '.MAIN_DB_PREFIX.'commande_fournisseur'
    ." WHERE entity IN (".getEntity('supplier_order').") AND ref LIKE '(PROV%'");
 ...
 * avant la passe principale — improbable ici (aucune référence actuelle ne commence par
 * « COF »), mais la protection ne coûte rien et couvre les relances partielles.
 */
...
echo "Script      : renumérotation des commandes fournisseur (coupure ".CUTOFF.")\n";
...
foreach ($preRenames as $rowid => $tmpRef) {
    $sql = 'UPDATE '.MAIN_DB_PREFIX."commande_fournisseur SET ref = '".$db->escape($tmpRef)."'"
...
foreach ($todo as $rowid => $target) {
    $sql = 'UPDATE '.MAIN_DB_PREFIX."commande_fournisseur SET ref = '".$db->escape($target)."'"
        .' WHERE rowid = '.((int) $rowid);
    if (!$db->query($sql)) {
        $failed++;
...
// La dernière référence de la série en cours, pour le compteur du modèle COF.
...
if ($resql && ($obj = $db->fetch_object($resql))) {
    echo "Dernière référence de la série en cours : ".$obj->ref
        ." — le modèle COF d'aerotoolbox reprendra après elle.\n";
}
